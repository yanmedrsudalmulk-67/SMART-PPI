...
import { 
  ...
  Camera, Upload, Plus, Edit2, Trash2, X, Settings, AlertCircle, Signature, Bed
} from 'lucide-react';
...
const initialUnits = [
  ...
  'Ranap Khadijah', 'Ranap Usman', 'Rawat Jalan'
];
...
const groupedChecklist = [
  {
    ...
    title: 'Ruang Isolasi',
    items: [
      { id: 'a_1', label: 'Tekanan Negatif' },
      { id: 'a_2', label: 'Pertukaran udara > 12 kali/jam' },
      { id: 'a_3', label: 'Suhu ruangan 22°C – 25°C' },
      { id: 'a_4', label: 'Kelembaban 40–60%' },
      { id: 'a_5', label: 'Kondisi pintu selalu tertutup' },
    ]
  },
  {
    id: 'B',
    ...
    items: [
      { id: 'b_1', label: 'Tersedia wastafel' },
      { id: 'b_2', label: 'Tersedia sabun cuci tangan' },
      { id: 'b_3', label: 'Tersedia tissue' },
      { id: 'b_4', label: 'Tersedia handrub' },
    ]
  },
  {
    id: 'C',
    title: 'Fasilitas APD',
    items: [
      { id: 'c_1', label: 'Tersedia sarung tangan' },
      { id: 'c_2', label: 'Tersedia masker bedah' },
      { id: 'c_3', label: 'Tersedia masker N-95' },
      { id: 'c_4', label: 'Tersedia kaca mata pelindung' },
      { id: 'c_5', label: 'Tersedia apron' },
      { id: 'c_6', label: 'Tersedia sepatu boot' },
      { id: 'c_7', label: 'Tersedia penutup kepala' },
    ]
  },
  {
    id: 'D',
    title: 'Petugas Isolasi',
    items: [
      { id: 'd_1', label: 'Melakukan kebersihan tangan 5 momen' },
      { id: 'd_2', label: 'Menggunakan APD saat melakukan tindakan di ruang isolasi' },
      { id: 'd_3', label: 'Membuang limbah sesuai standar' },
      { id: 'd_4', label: 'Menempatkan linen kotor pada tempatnya' },
      { id: 'd_5', label: 'Memberikan edukasi pada pasien dan keluarga pasien' },
    ]
  }
];
...
export default function RuangIsolasiPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const uploadedUrls = await uploadImagesToSupabase(supabase, images, 'ruang_isolasi', 'audit');
      
      const payload_data_indikator: Record<string, any> = {};
      groupedChecklist.forEach(group => {
      ...
      const payload = {
        ...
        observer,
        unit,
        data_indikator: payload_data_indikator,
        jumlah_dinilai: stats.dinilai,
        jumlah_patuh: stats.patuh,
        jumlah_tidak_patuh: stats.tidakPatuh,
      ...
      if (err.message && err.message.includes('row-level security policy')) {
        alert('Gagal menyimpan: Akses Ditolak oleh keamanan Supabase (RLS). Harap nonaktifkan Row-Level Security (RLS) pada tabel "audit_ruang_isolasi" atau tambahkan Policy (Insert) di dashboard Supabase agar aplikasi bisa mengisi data.');
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-16 px-4 sm:px-0">
      ...
      <div className="flex items-center gap-6 mb-8 relative bg-navy-dark/90 backdrop-blur-xl py-6 z-10 -mx-4 px-4 sm:mx-0 sm:px-0 border-b border-white/5">
        ...
        <div>
          <h1 className="text-[24px] sm:text-[30px] font-heading font-bold tracking-tight text-white drop-shadow-[0_0_25px_rgba(59,130,246,0.5)]">Input Audit Ruang Isolasi</h1>
          <p className="text-[12px] sm:text-[15px] font-bold uppercase tracking-[0.1em] text-blue-400 mt-1">Audit kepatuhan fasilitas dan pelaksanaan PPI pada ruang isolasi.</p>
        </div>
      </div>

      <div className="space-y-6">
        {/* SECTION 1: Waktu Observasi */}
        <div className="glass-card p-6 rounded-[24px] border border-white/5 relative overflow-hidden group hover:border-blue-500/20 transition-all duration-500">
          ...
          <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-blue-400 mb-6 group-hover:text-blue-300 transition-colors duration-500">
            <Clock className="w-4 h-4" /> Waktu Observasi
          </h2>
        ...
        <div className="glass-card p-6 rounded-[24px] border border-white/5 group hover:border-blue-500/20 transition-all duration-500 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-6">
             <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-indigo-400 group-hover:text-indigo-300 transition-colors duration-500">
               <User className="w-4 h-4" /> Profil Audit
             </h2>
             <button 
        ...
        {/* SECTION 3: Checklist Audit Ruang Isolasi */}
        <div className="space-y-4">
          <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-emerald-400 mb-2 pl-2">
            <Bed className="w-5 h-5" /> Ceklist Audit Ruang Isolasi
          </h2>
          
          {groupedChecklist.map((group) => {
            const isOpen = openSections.includes(group.id);
            return (
              <div key={group.id} className="glass-card rounded-[2rem] border border-white/5 overflow-hidden transition-all duration-300">
                <button 
                  onClick={() => toggleSection(group.id)}
                  className="w-full bg-white/5 p-5 flex items-center justify-between hover:bg-white/10 transition-colors"
                >
                  <span className="text-sm font-bold tracking-widest text-white">{group.title}</span>
                ...
                <AnimatePresence>
                  {isOpen && (
                    ...
                    >
                      <div className="p-5 border-t border-white/5 space-y-4 bg-navy-dark/30">
                        {group.items.map((item) => (
                          <div key={item.id} className="bg-navy-dark/50 border border-white/5 p-5 rounded-2xl flex flex-col gap-5 hover:border-white/10 transition-colors shadow-sm">
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                              <p className="text-sm font-medium text-slate-300 leading-relaxed md:max-w-[50%]">{item.label}</p>
                            ...
                            <div>
                              <label className="block text-[9px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 ml-1">Keterangan</label>
                              <input 
                                type="text"
                                placeholder="Tambah catatan untuk item ini..."
                                value={notes[item.id] || ''}
                                onChange={(e) => handleNoteChange(item.id, e.target.value)}
                                className="w-full bg-navy-dark/50 border border-white/5 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-blue-500/50 shadow-inner transition-all"
                              />
                            </div>
                          </div>
                        ))}
        ...
        <div className="glass-card p-8 rounded-[2.5rem] border border-white/5 relative overflow-hidden group hover:border-blue-500/20 transition-all duration-500">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
            ...
            <div className="bg-navy-dark/50 rounded-2xl p-5 border border-white/5 flex flex-col items-center justify-center text-center shadow-inner shadow-black/20">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500">Jumlah Patuh</span>
            </div>
            
            <div className="bg-navy-dark/50 rounded-2xl p-5 border border-white/5 flex flex-col items-center justify-center text-center shadow-inner shadow-black/20">
              <span className="text-3xl font-heading font-bold text-red-400 mb-1.5">{stats.tidakPatuh}</span>
              <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500">Tidak Patuh</span>
            </div>
        ...
        <div className="glass-card p-6 rounded-[24px] border border-white/5 relative overflow-hidden group hover:border-amber-500/20 transition-all duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <textarea 
                ...
                placeholder="Tuliskan temuan audit (opsional)..."
        ...
        <div className="glass-card p-6 rounded-[24px] border border-white/5 relative overflow-hidden group hover:border-purple-500/20 transition-all duration-500">
        ...
        <div className="glass-card p-6 rounded-[24px] border border-white/5 relative overflow-hidden group hover:border-blue-500/20 transition-all duration-500">
          ...
          <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-blue-400 mb-6">
            <Signature className="w-4 h-4" /> Tanda Tangan
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              <label className="block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 text-center">Tanda Tangan PJ Ruangan / Auditee</label>
              <div className="bg-white rounded-xl border border-white/20 p-2 h-40 shadow-inner">
                <SignatureCanvas 
                  ref={sigPadPj}
                  canvasProps={{ className: "w-full h-full rounded-lg" }}
        ...
        <div className="flex gap-4 pt-4">
          <button
            ...
            className="flex-1 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white rounded-2xl py-4 font-bold text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(37,99,235,0.3)] hover:shadow-[0_0_30px_rgba(37,99,235,0.5)] transform hover:-translate-y-1 active:translate-y-0"
          >
            {isSubmitting ? (
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-heading font-bold text-white flex items-center gap-2">
                  <Settings className="w-5 h-5 text-blue-400" /> Kelola Observer
                </h3>
                <button 
              ...
              <div className="space-y-4">
                <div className="bg-navy-dark border border-white/5 rounded-2xl p-4 max-h-[40vh] overflow-y-auto custom-scrollbar">
                  ...
                  {observers.length === 0 && (
                    <p className="text-center text-sm text-slate-500 py-4">Belum ada observer</p>
                  )}
                </div>

                <div className="pt-4 border-t border-white/10 space-y-3">
                  <label className="block text-xs font-bold uppercase tracking-widest text-slate-400">
                    {editObserverId ? 'Edit Observer' : 'Tambah Observer Baru'}
                  </label>
                  <div className="flex gap-2">
                    <input 
                      ...
                      placeholder="Nama & Gelar IPCN/IPCLN..."
